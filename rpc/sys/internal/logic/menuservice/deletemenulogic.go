...
func (l *DeleteMenuLogic) DeleteMenu(in *sysclient.DeleteMenuReq) (*sysclient.DeleteMenuResp, error) {
	for _, id := range in.Ids {
		...
		menu, err := q.WithContext(l.ctx).Where(query.SysMenu.ID.Eq(id)).First()
		...
		// 2.查询菜单是否菜单已分配
		count, err = query.SysRoleMenu.WithContext(l.ctx).Where(query.SysRoleMenu.MenuID.Eq(id)).Count()
		if err != nil {
		...
		_, _ = l.svcCtx.Redis.HdelCtx(l.ctx, l.svcCtx.RedisKey+"api_url", menu.APIURL)
	}
