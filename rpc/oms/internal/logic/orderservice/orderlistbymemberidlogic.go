...
func (l *OrderListByMemberIdLogic) OrderListByMemberId(in *omsclient.OrderListByMemberIdReq) (*omsclient.OrderListByMemberIdResp, error) {
	...
	//todo 字段调整
	var list []*omsclient.OrderListByMemberIdData
	for _, order := range *orderList {
		...
		var goodList []*omsclient.GoodsListByMemberIdData
		for _, orderItem := range *productListByOrderId {
			goodList = append(goodList, &omsclient.GoodsListByMemberIdData{
		...
		//todo 字段调整
		list = append(list, &omsclient.OrderListByMemberIdData{
			Id:              int32(order.Id),
