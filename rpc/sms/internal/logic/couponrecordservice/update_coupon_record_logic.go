...
func (l *UpdateCouponRecordLogic) UpdateCouponRecord(in *smsclient.UpdateCouponRecordReq) (*smsclient.UpdateCouponRecordResp, error) {
	...
	// 1.查询优惠券领取记录是否已存在
	coupon, err := q.WithContext(l.ctx).Where(q.MemberID.Eq(in.MemberId), q.CouponID.Eq(in.CouponId)).First()
	...
	c := query.SmsCoupon
	smsCoupon, err := c.WithContext(l.ctx).Where(c.ID.Eq(in.CouponId)).First()
	if err != nil {
	...
	_, err = c.WithContext(l.ctx).Where(c.ID.Eq(in.CouponId)).Update(c.UsedCount, count)
