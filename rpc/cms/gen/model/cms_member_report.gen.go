...
// CmsMemberReport mapped from table <cms_member_report>
type CmsMemberReport struct {
	ID               int64     `gorm:"column:id;primaryKey;autoIncrement:true;comment:编号" json:"id"`                                  // 编号
	ReportType       int32     `gorm:"column:report_type;not null;default:1;comment:举报类型：0->商品评价；1->话题内容；2->用户评论" json:"report_type"` // 举报类型：0->商品评价；1->话题内容；2->用户评论
	...
	ReportStatus     int32     `gorm:"column:report_status;not null;default:1;comment:举报状态：0->未处理；1->已处理" json:"report_status"`       // 举报状态：0->未处理；1->已处理
	HandleStatus     int32     `gorm:"column:handle_status;not null;default:1;comment:处理结果：0->无效；1->有效；2->恶意" json:"handle_status"`   // 处理结果：0->无效；1->有效；2->恶意
