...
var (
	...
	omsOrderReturnReasonRowsExpectAutoSet   = strings.Join(stringx.Remove(omsOrderReturnReasonFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	omsOrderReturnReasonRowsWithPlaceHolder = strings.Join(stringx.Remove(omsOrderReturnReasonFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	OmsOrderReturnReason struct {
		Id         int64          `db:"id"`
		Name       sql.NullString `db:"name"` // 退货类型
		Sort       sql.NullInt64  `db:"sort"`
		Status     sql.NullInt64  `db:"status"`      // 状态：0->不启用；1->启用
