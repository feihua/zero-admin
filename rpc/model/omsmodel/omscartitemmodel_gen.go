...
var (
	...
	omsCartItemRowsExpectAutoSet   = strings.Join(stringx.Remove(omsCartItemFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	omsCartItemRowsWithPlaceHolder = strings.Join(stringx.Remove(omsCartItemFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	OmsCartItem struct {
		...
		ProductSkuId      sql.NullInt64   `db:"product_sku_id"`
		MemberId          sql.NullInt64   `db:"member_id"`
		Quantity          sql.NullInt64   `db:"quantity"`            // 购买数量
		Price             sql.NullFloat64 `db:"price"`               // 添加到购物车的价格
		ProductPic        sql.NullString  `db:"product_pic"`         // 商品主图
		ProductName       sql.NullString  `db:"product_name"`        // 商品名称
		ProductSubTitle   sql.NullString  `db:"product_sub_title"`   // 商品副标题（卖点）
		ProductSkuCode    sql.NullString  `db:"product_sku_code"`    // 商品sku条码
		MemberNickname    sql.NullString  `db:"member_nickname"`     // 会员昵称
		CreateDate        sql.NullTime    `db:"create_date"`         // 创建时间
		ModifyDate        sql.NullTime    `db:"modify_date"`         // 修改时间
		DeleteStatus      int64           `db:"delete_status"`       // 是否删除
		ProductCategoryId sql.NullInt64   `db:"product_category_id"` // 商品分类
