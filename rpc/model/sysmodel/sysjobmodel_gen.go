...
type (
	...
	SysJob struct {
		Id             int64          `db:"id"`               // 编号
		JobName        sql.NullString `db:"job_name"`         // 职位名称
...
func (m *defaultSysJobModel) Insert(ctx context.Context, data *SysJob) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.JobName, data.OrderNum, data.CreateBy, data.LastUpdateBy, data.LastUpdateTime, data.DelFlag, data.Remarks)
	return ret, err
}
...
func (m *defaultSysJobModel) Update(ctx context.Context, data *SysJob) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.JobName, data.OrderNum, data.CreateBy, data.LastUpdateBy, data.LastUpdateTime, data.DelFlag, data.Remarks, data.Id)
	return err
}
