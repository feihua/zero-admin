...
var (
	...
	smsFlashPromotionSessionRowsExpectAutoSet   = strings.Join(stringx.Remove(smsFlashPromotionSessionFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	smsFlashPromotionSessionRowsWithPlaceHolder = strings.Join(stringx.Remove(smsFlashPromotionSessionFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	SmsFlashPromotionSession struct {
		Id         int64          `db:"id"`          // 编号
		Name       sql.NullString `db:"name"`        // 场次名称
		StartTime  sql.NullString `db:"start_time"`  // 每日开始时间
		EndTime    sql.NullString `db:"end_time"`    // 每日结束时间
		Status     sql.NullInt64  `db:"status"`      // 启用状态：0->不启用；1->启用
