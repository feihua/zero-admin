...
var (
	...
	smsCouponProductCategoryRelationRowsExpectAutoSet   = strings.Join(stringx.Remove(smsCouponProductCategoryRelationFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	smsCouponProductCategoryRelationRowsWithPlaceHolder = strings.Join(stringx.Remove(smsCouponProductCategoryRelationFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	SmsCouponProductCategoryRelation struct {
		...
		ProductCategoryName sql.NullString `db:"product_category_name"` // 产品分类名称
		ParentCategoryName  sql.NullString `db:"parent_category_name"`  // 父分类名称
	}
)
