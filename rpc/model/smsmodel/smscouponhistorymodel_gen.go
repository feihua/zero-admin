...
var (
	...
	smsCouponHistoryRowsExpectAutoSet   = strings.Join(stringx.Remove(smsCouponHistoryFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	smsCouponHistoryRowsWithPlaceHolder = strings.Join(stringx.Remove(smsCouponHistoryFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	SmsCouponHistory struct {
		...
		CouponCode     sql.NullString `db:"coupon_code"`
		MemberNickname sql.NullString `db:"member_nickname"` // 领取人昵称
		GetType        sql.NullInt64  `db:"get_type"`        // 获取类型：0->后台赠送；1->主动获取
		CreateTime     sql.NullTime   `db:"create_time"`
		UseStatus      sql.NullInt64  `db:"use_status"` // 使用状态：0->未使用；1->已使用；2->已过期
		UseTime        sql.NullTime   `db:"use_time"`   // 使用时间
		OrderId        sql.NullInt64  `db:"order_id"`   // 订单编号
		OrderSn        sql.NullString `db:"order_sn"`   // 订单号码
	}
)
