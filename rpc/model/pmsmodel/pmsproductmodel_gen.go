...
var (
	...
	pmsProductRowsExpectAutoSet   = strings.Join(stringx.Remove(pmsProductFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	pmsProductRowsWithPlaceHolder = strings.Join(stringx.Remove(pmsProductFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"

	cacheGozeroPmsProductIdPrefix = "cache:gozero:pmsProduct:id:"
)

type (
	pmsProductModel interface {
		Insert(ctx context.Context, data *PmsProduct) (sql.Result, error)
		InsertTx(ctx context.Context, session sqlx.Session, data *PmsProduct) (sql.Result, error)
		FindOne(ctx context.Context, id int64) (*PmsProduct, error)
		Update(ctx context.Context, data *PmsProduct) error
		UpdateTx(ctx context.Context, session sqlx.Session, data *PmsProduct) error
...
func newPmsProductModel(conn sqlx.SqlConn, c cache.CacheConf) *defaultPmsProductModel {
...
func (m *defaultPmsProductModel) Delete(ctx context.Context, id int64) error {
	gozeroPmsProductIdKey := fmt.Sprintf("%s%v", cacheGozeroPmsProductIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, gozeroPmsProductIdKey)
	return err
}

func (m *defaultPmsProductModel) DeleteTx(ctx context.Context, session sqlx.Session, id int64) error {
	gozeroPmsProductIdKey := fmt.Sprintf("%s%v", cacheGozeroPmsProductIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		if session != nil {
...
func (m *defaultPmsProductModel) FindOne(ctx context.Context, id int64) (*PmsProduct, error) {
	gozeroPmsProductIdKey := fmt.Sprintf("%s%v", cacheGozeroPmsProductIdPrefix, id)
	var resp PmsProduct
	err := m.QueryRowCtx(ctx, &resp, gozeroPmsProductIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultPmsProductModel) UpdateTx(ctx context.Context, session sqlx.Session, data *PmsProduct) error {
	gozeroPmsProductIdKey := fmt.Sprintf("%s%v", cacheGozeroPmsProductIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, pmsProductRowsWithPlaceHolder)
		if session != nil {
...
func (m *defaultPmsProductModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheGozeroPmsProductIdPrefix, primary)
}

func (m *defaultPmsProductModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", pmsProductRows, m.table)
