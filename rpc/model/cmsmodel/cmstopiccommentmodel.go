...
type (
	...
	CmsTopicCommentModel interface {
		...
		FindAll(ctx context.Context, Current int64, PageSize int64) (*[]CmsTopicComment, error)
...
func (m *customCmsTopicCommentModel) FindAll(ctx context.Context, Current int64, PageSize int64) (*[]CmsTopicComment, error) {

	query := fmt.Sprintf("select %s from %s limit ?,?", cmsTopicCommentRows, m.table)
...
func (m *customCmsTopicCommentModel) DeleteByIds(ctx context.Context, ids []int64) error {
	// 拼接占位符 "?"
