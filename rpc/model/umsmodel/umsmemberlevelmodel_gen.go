...
var (
	...
	umsMemberLevelRowsExpectAutoSet   = strings.Join(stringx.Remove(umsMemberLevelFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), ",")
	umsMemberLevelRowsWithPlaceHolder = strings.Join(stringx.Remove(umsMemberLevelFieldNames, "`id`", "`create_time`", "`update_time`", "`create_at`", "`update_at`"), "=?,") + "=?"
)
...
type (
	...
	UmsMemberLevel struct {
		...
		GrowthPoint           sql.NullInt64   `db:"growth_point"`
		DefaultStatus         sql.NullInt64   `db:"default_status"`          // 是否为默认等级：0->不是；1->是
		FreeFreightPoint      sql.NullFloat64 `db:"free_freight_point"`      // 免运费标准
		CommentGrowthPoint    sql.NullInt64   `db:"comment_growth_point"`    // 每次评价获取的成长值
		PriviledgeFreeFreight sql.NullInt64   `db:"priviledge_free_freight"` // 是否有免邮特权
		PriviledgeSignIn      sql.NullInt64   `db:"priviledge_sign_in"`      // 是否有签到特权
		PriviledgeComment     sql.NullInt64   `db:"priviledge_comment"`      // 是否有评论获奖励特权
		PriviledgePromotion   sql.NullInt64   `db:"priviledge_promotion"`    // 是否有专享活动特权
		PriviledgeMemberPrice sql.NullInt64   `db:"priviledge_member_price"` // 是否有会员价格特权
		PriviledgeBirthday    sql.NullInt64   `db:"priviledge_birthday"`     // 是否有生日特权
